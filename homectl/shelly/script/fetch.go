...
func fetchFromOneDevice(ctx context.Context, log logr.Logger, via types.Channel, device *shelly.Device, args []string) (any, error) {
	if len(args) == 1 {
		scripts, err := script.DeviceStatus(ctx, device, via)
		if err != nil {
			return nil, err
		}
...
func fetchOneScript(ctx context.Context, log logr.Logger, via types.Channel, device *shelly.Device, args []string) error {
	...
	for _, l := range loaded {
		if l.Name == args[0] {
			...
			code, err := script.Download(ctx, via, device, args[0], l.Id)
			if err != nil {
				log.Error(err, "Unable to get code", "name", args[0])
				return err
			}
			// store code string in file by name, in a folder named by the device id
			os.MkdirAll(device.Id(), 0755)
			os.WriteFile(filepath.Join(device.Id(), args[0]), []byte(code), 0644)
			return nil
		}
	}
	return nil
