...
func init() {
	...
	runCmd.PersistentFlags().Float64Var(&options.Flags.PoolSolarMinVolumeTurnover, "pool-solar-min-volume-turnover", 5, "Pool volume turnovers/day for soft stop: stop when reached AND solar gone (0 = no soft stop)")
	runCmd.PersistentFlags().Float64Var(&options.Flags.PoolSolarMaxVolumeTurnover, "pool-solar-max-volume-turnover", 7, "Pool volume turnovers/day for hard ceiling: always stop when reached (0 = no ceiling)")
